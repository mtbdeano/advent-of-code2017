...
def day8_run(s):
    ...
    for ins in s.split('\n'):
        ...
        r = registers.get(tokens[0], 0)
        registers[tokens[0]] = r  # lazy set
        print('r {} was {} updown {} result {}'.format(tokens[0], r, updown, r + updown * int(tokens[2])))
        r += updown * int(tokens[2])
        if tokens[5] == '>':
            if registers.get(tokens[4], 0) > int(tokens[6]):
                registers[tokens[0]] = r
        elif tokens[5] == '<':
            if registers.get(tokens[4], 0) < int(tokens[6]):
                registers[tokens[0]] = r        
        elif tokens[5] == '<=':
...
def main():
    ...
    print('highest is {}'.format(max(results.values())))
    result = day8_max(PROGRAM)
    print('biggliest is {}'.format(result))
