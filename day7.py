...
class Node(object):
    ...
    @staticmethod
    def link_up():
        ''' once tree nodes are built and children set, fill in the parents '''
        [[Node.find_node(kid).add_parent(node.name) for kid in node.kids()] for name, node in NODES.items()]
